...
def optimize_image(fname: str):
    ...
    try:
        ...
        # If cwebp is not installed, copy original to .webp name to satisfy HTML references without crashing the build
        try:
            import shutil
            shutil.copy2(fname, outname)
            print(f"Bypassed optimization for {fname} (copied original as .webp fallback)")
        except Exception as copy_err:
            print(f"Could not fallback for {fname}: {copy_err}")
